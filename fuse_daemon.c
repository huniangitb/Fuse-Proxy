...
int g_passthrough_enabled = 0;
int g_preserve_perms = 0;
...
static void fuse_debug_log_cb(enum fuse_log_level level, const char *fmt, va_list ap) {
    ...
    char time_buf[64]; time_t now = time(NULL); struct tm tm_buf; localtime_r(&now, &tm_buf); strftime(time_buf, sizeof(time_buf), "%H:%M:%S", &tm_buf);
    const char* lvl = "?";
...
/* 修复：路径缓冲区使用 PATH_MAX，统一锁管理，宏末尾不自动解锁 */
...
static int vfs_getattr(const char *path, struct stat *st, struct fuse_file_info *fi) {
    ...
    if (lstat(p, st) == -1) {
        int err = errno;
        if (err == ENOENT && (vfs_is_virtual_ancestor(cfg, sub_path) || vfs_is_redirect_target(cfg, sub_path))) {
    ...
    else if (is_redir && S_ISDIR(st->st_mode)) st->st_mode = (st->st_mode & ~07777) | 0777;
    VFS_RETURN(0);
}
...
static int vfs_access(const char *path, int mask) {
    ...
    if (access(p, mask) == -1) {
        int err = errno; if (err == EIO) VFS_RETURN(-EIO);
...
static int vfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi, enum fuse_readdir_flags flags) {
    ...
    int skip_sandbox = (path_user_id < 0);
    DirHashTable ht; vfs_hash_init(&ht);
    DIR *dp = opendir(p);
    char item_sub_path[2048], item_real_path[2048];
    int is_root = (strcmp(sub_path, "/") == 0 || sub_path[0] == '\0');
    int is_android_dir = (!skip_sandbox && (strcmp(sub_path, "/Android") == 0 || strcmp(sub_path, "/Android/") == 0));
    int hide_others = (!skip_sandbox && current_pkg[0] && strcmp(current_pkg, "system") != 0 && strcmp(current_pkg, "unknown") != 0);
    int sandbox_enabled = (cfg && cfg->sandbox && hide_others);
    int has_data = 0, has_obb = 0, has_android = 0;
    ...
    if (dp) {
        ...
        while ((de = readdir(dp)) != NULL) {
            ...
            if (ctx->uid != 0 && vfs_sanitize_and_check_hidden(cfg, item_sub_path, NULL, dummy_path, sizeof(dummy_path), current_pkg, ctx->uid) != 0) continue;
            if (ctx->uid != 0 && vfs_is_redirect_target(cfg, item_sub_path)) continue;

            if (is_root && strcmp(de->d_name, "Android") == 0) has_android = 1;
            if (is_android_dir && sandbox_enabled) {
                if (strcmp(de->d_name, "data") == 0) has_data = 1;
                else if (strcmp(de->d_name, "obb") == 0) has_obb = 1;
                else continue;
            }
            ...
            if (vfs_hash_insert(&ht, de->d_name)) {
                ...
                } else filler(buf, de->d_name, NULL, 0, 0);
            }
        }
        closedir(dp);
    }
    
    if (is_root && sandbox_enabled && !has_android) { struct stat st_dir; memset(&st_dir, 0, sizeof(st_dir)); st_dir.st_mode = S_IFDIR | 0755; st_dir.st_uid = ctx->uid; st_dir.st_gid = ctx->gid; filler(buf, "Android", &st_dir, 0, 0); }
    if (is_android_dir && sandbox_enabled) {
...
static int vfs_open(const char *path, struct fuse_file_info *fi) {
    ...
    if (fd == -1) { int err = errno; if (err != ENOENT) vfs_log_io_err(current_pkg, "OPEN", sub_path, "errno=%d real='%s'", err, p); VFS_RETURN(-err); }
    fi->fh = fd;
    VFS_RETURN(0);
}
...
static int vfs_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
    ...
    if (ctx->uid != 0 && vfs_is_path_ro(cfg, sub_path)) VFS_RETURN(-EROFS);
    if (g_strip_o_direct && (fi->flags & O_DIRECT)) fi->flags &= ~O_DIRECT;
    ...
    if (fd == -1) {
        ...
        if (fd == -1) { int err = errno; if (err != EEXIST) vfs_log_io_err(current_pkg, "CREATE", sub_path, "errno=%d", err); VFS_RETURN(-err); }
    }
...
static int vfs_mkdir(const char *path, mode_t mode) {
    ...
    if (ret == -1) {
        ...
        if (ret == -1) { if (errno != EEXIST) vfs_log_io_err(current_pkg, "MKDIR", sub_path, "errno=%d", errno); VFS_RETURN(-errno); }
    }
...
static int vfs_rmdir(const char *path) {
    ...
    if (vfs_is_virtual_ancestor(cfg, sub_path) || vfs_is_redirect_target(cfg, sub_path)) {
        if (!vfs_is_virtual_dir_empty(cfg, sub_path, p, current_pkg)) VFS_RETURN(-ENOTEMPTY);
        int ret = rmdir(p);
        if (ret == -1 && errno != ENOENT) VFS_RETURN(-errno);
        VFS_RETURN(0);
    }
    int ret = rmdir(p);
    if (ret == -1) VFS_RETURN(-errno);
...
static int vfs_fallocate(const char *path, int mode, off_t offset, off_t length, struct fuse_file_info *fi) {
    (void)path;
    int ret = fallocate(fi->fh, mode, offset, length);
    if (ret == -1) return -errno;
    return 0;
}

static int vfs_lock(const char *path, struct fuse_file_info *fi, int cmd, struct flock *lock) {
    (void)path;
    int ret = fcntl(fi->fh, cmd, lock);
    if (ret == -1) return -errno;
    return 0;
}
...
static int vfs_opendir(const char *path, struct fuse_file_info *fi) {
    ...
    if (errno == ENOENT && (vfs_is_virtual_ancestor(cfg, sub_path) || vfs_is_redirect_target(cfg, sub_path))) {
        VFS_RETURN(0);
    }
    VFS_RETURN(-errno);
}
...
static int vfs_rename(const char *from, const char *to, unsigned int flags) {
    ...
    char sub_from[PATH_MAX], sub_to[PATH_MAX], p_from[PATH_MAX], p_to[PATH_MAX]; int u_from, u_to, redir_from, redir_to;
    ...
    vfs_log_io(cfg_from, pkg_from, ctx->uid, "RENAME_FROM", sub_from, p_from, 0);
    vfs_log_io(cfg_to, pkg_to, ctx->uid, "RENAME_TO", sub_to, p_to, 0);
    if (ctx->uid != 0 && (vfs_is_path_ro(cfg_from, sub_from) || vfs_is_path_ro(cfg_to, sub_to))) { config_unlock_read(); return -EROFS; }
...
static struct fuse_operations vfs_ops = {
    ...
    .release = vfs_release, .flush = vfs_flush, .fsync = vfs_fsync, .fallocate = vfs_fallocate,
    .lock = vfs_lock, .mkdir = vfs_mkdir, .unlink = vfs_unlink, .rmdir = vfs_rmdir,
