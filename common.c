...
int g_persist_all_logs = 0;
int g_strip_o_direct = 0;  
...
int mkdir_recursive(const char *dir) {
    char tmp[256], *p = NULL; snprintf(tmp, sizeof(tmp), "%s", dir);
...
int path_contains_invalid_chars(const char* path) {
    ...
    while (*p) {
        if (*p < 0x08) return 1;              
        if (*p > 0x09 && *p < 0x20) return 1; 
        if (*p == 0x7F) return 1;             

        // 检查 2 字节序列: C2 A0 或 C2 AD
        if (p[0] == 0xC2 && p[1] >= 0xA0 && p[1] <= 0xAD) {
            // 需要确保有足够的字节
            if (p[1] == 0xA0 || p[1] == 0xAD) return 1;
        }

        // 检查 3 字节序列: E2 80 80-8A, etc.
        if (p[0] == 0xE2 && p[1] == 0x80 && p[2] >= 0x80 && p[2] <= 0xAF) return 1;
        if (p[0] == 0xE2 && p[1] == 0x81 && p[2] >= 0xA0 && p[2] <= 0xAF) return 1;
        if (p[0] == 0xE1 && p[1] == 0xA0 && p[2] == 0x8E) return 1;
        if (p[0] == 0xEF && p[1] == 0xBB && p[2] == 0xBF) return 1;

        // 检查 4 字节序列: F3 A0 80 80-BF, etc.
        if (p[0] == 0xF3 && p[1] == 0xA0 && p[2] == 0x80 && p[3] >= 0x80 && p[3] <= 0xBF) return 1;
        if (p[0] == 0xF3 && p[1] == 0xA0 && p[2] == 0x81 && p[3] >= 0x80 && p[3] <= 0xAF) return 1;
...
void path_sanitize(char* out, size_t size, const char* path) {
    ...
    while (*p && o < max) {
        ...
        if (*p < 0x08) { matched = 1; skip_bytes = 1; }
        else if (*p > 0x09 && *p < 0x20) { matched = 1; skip_bytes = 1; }
        else if (*p == 0x7F) { matched = 1; skip_bytes = 1; }
        else if (p[0] == 0xC2 && (p[1] == 0xA0 || p[1] == 0xAD)) { matched = 1; skip_bytes = 2; }
        else if (p[0] == 0xE2 && p[1] == 0x80 && p[2] >= 0x80 && p[2] <= 0xAF) { matched = 1; skip_bytes = 3; }
        else if (p[0] == 0xE2 && p[1] == 0x81 && p[2] >= 0xA0 && p[2] <= 0xAF) { matched = 1; skip_bytes = 3; }
        else if (p[0] == 0xE1 && p[1] == 0xA0 && p[2] == 0x8E) { matched = 1; skip_bytes = 3; }
        else if (p[0] == 0xEF && p[1] == 0xBB && p[2] == 0xBF) { matched = 1; skip_bytes = 3; }
        else if (p[0] == 0xF3 && p[1] == 0xA0 && p[2] == 0x80 && p[3] >= 0x80 && p[3] <= 0xBF) { matched = 1; skip_bytes = 4; }
        else if (p[0] == 0xF3 && p[1] == 0xA0 && p[2] == 0x81 && p[3] >= 0x80 && p[3] <= 0xAF) { matched = 1; skip_bytes = 4; }
...
void log_internal(LogLevel level, const char* format, ...) {
    ...
    if (g_persist_all_logs) {
        if (level == LOG_LOGCAT) persist_log_to_disk("logcat", msg_buf, 1);
        else if (level == LOG_IO) persist_log_to_disk("io", msg_buf, 1);
        else persist_log_to_disk("sys", msg_buf, 1);
    }
